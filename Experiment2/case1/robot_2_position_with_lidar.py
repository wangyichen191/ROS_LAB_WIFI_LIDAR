...
class SubThenPub:
        def __init__(self,p,v):
                ...
                self.pub = rospy.Publisher('/robot_2/cmd_vel',Twist,queue_size=1)

                # self.__sub_ = rospy.Subscriber('/robot_2/scan',Odometry,self.update_robot_leader,queue_size=1)
                self.sub0 = rospy.Subscriber('/robot_0/odom',Odometry,self.update_robot_leader,queue_size=1)
                self.sub = rospy.Subscriber('/robot_2/scan',LaserScan,self.ScanCallback)
                self.selfsub = rospy.Subscriber('/robot_2/odom',Odometry,self.update_robot_me,queue_size=1)
                self.followsub = rospy.Subscriber('/robot_3/odom',Odometry,self.update_robot_follower,queue_size=1)
        ...
        # if lidar has found a car, output 'yes', else 'no'
                ...
                pos0_pos2 = calculate_distance(self.pos_leader,self.pos1)
                pos1_pos2 = calculate_distance(self.pos1,self.lidar_pos0)
                pos3_pos2 = calculate_distance(self.pos1,self.pos2)

                self.accel = self.p * ((pos0_pos2 - 2 * start_distance) + (pos1_pos2  - start_distance) + (-pos3_pos2 + start_distance)) + \
                             self.v * ((self.twist_leader.linear.x - self.twist1.linear.x) + (self.lidar_twist0.linear.x - self.twist1.linear.x) + (self.twist2.linear.x - self.twist1.linear.x))
                ...
                rospy.loginfo("pos0_pos2: %0.2f pos1_pos2: %0.2f pos3_pos2: %0.2f x: %0.2f y:%0.2f linear_speed: %0.2f angular_speed: %0.2f", \
                              pos0_pos2,pos1_pos2,pos3_pos2,self.pos1.position.x,self.pos1.position.y,self.twist1.linear.x,self.twist1.angular.z)

                rospy.loginfo("LEADER: x:%0.2f y:%0.2f linear_speed:%0.2f",self.lidar_pos0.position.x,self.lidar_pos0.position.y,self.lidar_twist0.linear.x)
        ...
        def update_robot_leader(self,data):
                self.pos_leader.position.x = data.pose.pose.position.x + 2 * start_distance
        ...
        def update_robot_me(self,data):
                ...
                ticks = time.time()
                position_x = data.pose.pose.position.x - 2*start_distance
        ...
        def update_robot_follower(self,data):
...
def main():
        rospy.init_node('robot_2_position_with_lidar',anonymous=True)
